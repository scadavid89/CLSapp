...
├── staticwebapp.config.json    Routing and role gating for Azure Static Web Apps
...
Target is Azure Static Web Apps. From the Azure portal, create a Static Web App and point it at this GitHub repo — Azure adds the `AZURE_STATIC_WEB_APPS_API_TOKEN` secret to the repository and can generate its own workflow. Delete the generated file and keep `.github/workflows/azure-static-web-apps.yml`, which builds explicitly so the failure lands in your log rather than inside the deploy action.

Build settings if asked: app location `/`, output location `dist`, api location blank until Phase 2.

Every pull request gets its own preview URL. It closes automatically when the PR does.
